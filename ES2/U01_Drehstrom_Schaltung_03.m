...
U1N = 21.8*exp(j*(-30*pi/180));               % gemessen
U2N = 25.3*exp(j*(-154.5*pi/180));               %
U3N = 25.6*exp(j*(94.5*pi/180));
...
I_1b = abs (I_1)
I_2 = 234e-3*exp(j*(angle(U2N/Z2N))) ;
I_2b = abs (I_2)
I_3 = 243e-3*exp(j*(angle(U3N/Z3N))) ;
I_3b = abs (I_3)
Phi_I_N = (angle(I_1 + I_2 + I_3));              %Phasenwinkel Nullleiter   

%I_N = I_1 + I_2 + I_3;                          % Strom Nulleiter
I_N = 298e-3*exp(j*Phi_I_N); 
I_Nb = abs (I_N)

% quiver U12, U23, U31
figure('Name', 'Schaltung 3'), title ('Schaltung 3')

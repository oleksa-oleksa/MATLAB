...
U1N = 20.9*exp(j*(-30*pi/180));               % gemessen
U2N = 21.7*exp(j*(-154.5*pi/180));               %
U3N = 23*exp(j*(94.5*pi/180));

% SCHALTUNG 3

%Z_S2 = R2;          % komplexer Widerstand Schaltung 1

Z1N = R1;                     % Widerstand Strang 1, 2, 3
Z2N = R2;
Z3N = R3; 
...
I_1 = 291e-3*exp(j*(angle(U1N/Z1N))) ;
I_1b = abs (I_1)
I_2 = 244e-3*exp(j*(angle(U2N/Z2N))) ;
I_2b = abs (I_2)
I_3 = 172e-3*exp(j*(angle(U3N/Z3N))) ;
I_3b = abs (I_3)
Phi_I_N = (angle(I_1 + I_2 + I_3));              %Phasenwinkel Nullleiter   

%I_N = I_1 + I_2 + I_3;                          % Strom Nulleiter
I_N = 110e-3*exp(j*Phi_I_N); 
I_Nb = abs (I_N)

% quiver U12, U23, U31
figure('Name', 'Schaltung 5'), title ('Schaltung 5')

...
U1N = 23.6*exp(j*(-30*pi/180));               % gemessen
U2N = 23.6*exp(j*(-154.5*pi/180));               %
U3N = 23.8*exp(j*(94.5*pi/180));

% SCHALTUNG 3

%Z_S2 = R2;          % komplexer Widerstand Schaltung 1

Z1N = R2 + (1/(j*w*C));                     % Widerstand Strang 1, 2, 3
Z2N = R2 + 1/(j*w*C);
Z3N = R2 + 1/(j*w*C); 
...
I_1 = 167e-3*exp(j*(angle(U1N/Z1N))) ;
I_1b = abs (I_1)
I_2 = 167e-3*exp(j*(angle(U2N/Z2N))) ;
I_2b = abs (I_2)
I_3 = 174e-3*exp(j*(angle(U3N/Z3N))) ;
I_3b = abs (I_3)
Phi_I_N = (angle(I_1 + I_2 + I_3));              %Phasenwinkel Nullleiter   

%I_N = I_1 + I_2 + I_3;                          % Strom Nulleiter
I_N = 7e-3*exp(j*Phi_I_N); 
I_Nb = abs (I_N)
